...
def run_git(args, cwd=None, check=True):
    ...
    Args:
        args: git 子命令参数列表，如 ['status']
        cwd: 执行目录。None 时走当前进程工作目录（推荐调用方显式传入 project_dir）。
        check: 是否对非 0 exit code 打日志。
    """
    try:
        proc = subprocess.run(
            ["git"] + list(args),
...
def _is_ignored(rel_path, extra_suffixes=None, extra_names=None):
    ...
    merged_sf += list(EXTRA_extra_suffixes)
            ...
            try:
                ...
                for dp, dns, fns in os.walk(root):
                    dns[:] = [d for d in dns if d not in {
                        ...
                        '.svn', '.hg', 'chat_history', 'tmp_uploads',
                        'dist', 'build', 'backup', 'download', 'output',
                        '.idea', '.vscode', '.nuitka_cache', '.pytest_cache',
                        '.mypy_cache', '.ruff_cache', '.eggs', 'ppswsh_fix',
                    }]
                    for fn in fns:
                        rel = os.path.relpath(os.path.join(dp, fn), root)
                        _all.append(rel.replace(os.sep, '/'))
                if _all:
                    log_fn('📂 项目目录内共 {} 个文件，全部被过滤。'.format(len(_all)), 'WARN')
                    for rel in _all[:30]:
                        log_fn('  ↳ 已过滤：{}'.format(rel), 'WARN')
                    if len(_all) > 30:
                        log_fn('  ↳ ...（还有 {} 个）'.format(len(_all) - 30), 'WARN')
                else:
                    log_fn('📂 项目目录为空或无可上传文件。', 'WARN')
            except Exception:
                log_fn('📂 项目目录为空或无可上传文件。', 'WARN')
...
def safe_pull_rebase(cwd, remote, branch, log_fn=None):
    """带冲突自动处理的 pull --rebase。

    - 若有未追踪文件会被 checkout 覆盖：先 `git stash -u`，pull，再 `git stash pop`。
    - 尽量使用 `--autostash`（Git 2.27+ 内置），不支持则退回到手动 stash 流程。
    Returns:
        rc, out, err
    """
    rc0, out0, err0 = run_git(
        ['pull', '--rebase', '--autostash', remote, branch], cwd=cwd)
    if rc0 == 0:
        if log_fn:
            log_fn('✅ pull --rebase --autostash 成功', 'SUCCESS')
        return rc0, out0, err0
    if 'autostash' in (err0 or '').lower() and 'unknown' in (err0 or '').lower():
        rc_st, out_st, err_st = run_git(['stash', 'push', '-u'], cwd=cwd)
        if log_fn:
            for line in (out_st + err_st).splitlines()[:10]:
                log_fn(line, 'INFO')
        rc1, out1, err1 = run_git(
            ['pull', '--rebase', remote, branch], cwd=cwd)
        rc_sp, out_sp, err_sp = run_git(['stash', 'pop'], cwd=cwd)
        if rc_sp != 0 and log_fn:
            log_fn('⚠️ stash pop 冲突，请手动解决。', 'WARN')
        if log_fn:
            if rc1 == 0:
                log_fn('✅ pull --rebase 成功（自动处理了未追踪文件）', 'SUCCESS')
            else:
                for line in (out1 + err1).splitlines()[:30]:
                    log_fn(line, 'WARN')
        return rc1, out1, err1
    if log_fn:
        for line in (out0 + err0).splitlines()[:30]:
            log_fn(line, 'WARN')
    return rc0, out0, err0
...
_DEFAULT_CONFIG = {"folders": [], "github_token": ""}
...
def _read_remote_origin_url(cwd=None) -> str:
    """从 git config 读取 remote.origin.url，失败返回空字符串。"""
    try:
        rc, out, _ = run_git(['config', '--get', 'remote.origin.url'], cwd=cwd)
        if rc == 0:
            return out.strip()
    except Exception:
        pass
...
def _ensure_github_identity(remote_url: str, cwd=None) -> None:
    ...
    - 同时在当前项目（cwd）和全局（--global）都做一次，保证局部配置不覆盖全局。
    """
    if not remote_url or "github.com" not in remote_url.lower():
        return
    # 本地项目级配置优先
    rc_name, out_name, _ = run_git(["config", "--get", "user.name"], cwd=cwd)
    if rc_name != 0 or not out_name.strip():
        rc_name_g, out_name_g, _ = run_git(["config", "--global", "--get", "user.name"])
        if rc_name_g == 0 and out_name_g.strip():
            run_git(["config", "user.name", out_name_g.strip()], cwd=cwd)
        else:
            run_git(["config", "--global", "user.name", "github-user"])
            run_git(["config", "user.name", "github-user"], cwd=cwd)
    # 邮箱检查：本地 + 全局
    rc_email, out_email, _ = run_git(["config", "--get", "user.email"], cwd=cwd)
    rc_email_g, out_email_g, _ = run_git(["config", "--global", "--get", "user.email"])
    need_noreply = False
    if (rc_email != 0 or not out_email.strip()) and (
            rc_email_g != 0 or not out_email_g.strip()):
        need_noreply = True
    else:
        # 本地或全局有一个配置了非 noreply 的邮箱，都算"不安全"
        for mail in (out_email.strip() if rc_email == 0 else "",
                     out_email_g.strip() if rc_email_g == 0 else ""):
            cur = mail.lower()
            if not cur:
                continue
    ...
    if need_noreply:
        ...
        nm = out_n.strip() if rc_n == 0 and out_n.strip() else "github-user"
        safe_email = _gh_compliant_email(nm)
        run_git(["config", "--global", "user.email", safe_email])
        if cwd is not None:
            run_git(["config", "user.email", safe_email], cwd=cwd)


def _working_tree_is_dirty(cwd=None) -> bool:
...
def _collect_changed_files(cwd=None) -> list:
...
class GitManagerApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # 如果配置里已有已保存的文件夹，启动时默认切到最后一个（最近使用）
        folders = [p for p in (self._app_cfg.get("folders", []) or [])
                   if isinstance(p, str) and p.strip() and os.path.isdir(p)]
        if folders:
            self.project_dir = folders[-1]
        else:
            self.project_dir = PROGRAM_DIR
        self.folder_var = tk.StringVar(value=self.project_dir if folders else "")
        ...
        # 启动即最大化（兼容 Windows 和其他平台）
        try:
            if is_windows():
                self.state("zoomed")  # Windows 最大化
            else:
                self.attributes("-zoomed", True) or self.state("normal")
        except Exception:
            try:
                self.state("zoomed")
            except Exception:
        ...
        self._status_vars = {
            ...
            "repo": tk.BooleanVar(value=is_git_repo(self.project_dir)),
        }
        self._build_ui()
        self._refresh_status()
        self._log("ℹ️ 程序已启动，工作目录：{}".format(self.project_dir), "INFO")
    ...
    def _build_ui(self):
        ...
        # 文件夹下拉框 + 查找按钮
        tk.Label(top, text="  📂 文件夹:", bg=TITLE_BG, fg=TITLE_FG,
                 font=("Microsoft YaHei", 9)).pack(side="left", padx=(10, 4))
        self.folder_combo = ttk.Combobox(top, textvariable=self.folder_var,
                                         width=52, state="readonly",
                                         font=("Microsoft YaHei", 9))
        # 下拉展开最大宽度限制，防止超长路径撑爆窗口
        try:
            self.folder_combo.configure(postcommand=lambda: None)
        except Exception:
            pass
        self.folder_combo.pack(side="left", padx=(0, 4))
        self._refresh_folder_combo()
        self.folder_combo.bind("<<ComboboxSelected>>", self._on_folder_select)
        b_find = tk.Button(top, text="🔍 查找", bg=BTN_COLOR1, fg="white",
                  relief="flat", activebackground=BTN_COLOR1, activeforeground="white",
                  font=("Microsoft YaHei", 9), padx=6, pady=2,
                  command=self._cmd_find_folder)
        b_find.pack(side="left", padx=4)
        ToolTip(b_find, "选择并保存一个 Git 工作文件夹")
        b_manage = tk.Button(top, text="📋 管理", bg=BTN_COLOR2, fg="white",
                  relief="flat", activebackground=BTN_COLOR2, activeforeground="white",
                  font=("Microsoft YaHei", 9), padx=6, pady=2,
                  command=self._cmd_manage_folders)
        b_manage.pack(side="left", padx=4)
        ToolTip(b_manage, "管理已保存的文件夹列表（删除、排序）")

        # 顶部右按钮（自适应大小，使用紧凑内边距）
        for txt, cmd, bg, hint in [
            ("❓ 帮助", self._help_me, BTN_COLOR2, "查看使用说明"),
            ("ℹ️ 关于", self._about_me, BTN_COLOR2, "查看程序作者信息"),
            ("🌐 EN", self._toggle_lang, BTN_COLOR1, "切换中文/英文"),
            ("🌙 暗黑", self._toggle_dark, BTN_COLOR1, "切换暗黑/明亮主题"),
            ("🎨 皮肤", self._cycle_theme, BTN_COLOR1, "循环切换皮肤主题"),
        ]:
            b = tk.Button(top, text=txt, bg=bg, fg="white",
                          relief="flat",
    ...
    def _refresh_status(self):
        ...
        self._status_vars["repo"].set(is_git_repo(self.project_dir))
    ...
    def _refresh_filebox(self):
        ...
        files = scan_project_files(self.project_dir,
    ...
    def _exec_git(self, args, cwd=None, label="git", check=True):
        ...
        rc, out, err = run_git(args, cwd=cwd or self.project_dir)
    ...
    def _refresh_folder_combo(self):
        folders = self._app_cfg.get("folders", []) or []
        # 过滤掉已不存在的路径，防止下拉框显示无效条目
        cleaned = [p for p in folders if isinstance(p, str) and p.strip()]
        if cleaned != folders:
            self._app_cfg["folders"] = cleaned
            save_config(self._app_cfg)
        self.folder_combo["values"] = cleaned
        # 如果当前选择无效，则回退到最后一个有效条目
        if not self.folder_var.get() and cleaned:
            self.folder_var.set(cleaned[-1])

    def _on_folder_select(self, event=None):
        """下拉框选择文件夹：立即切换，并记住最后使用的文件夹（持久化）。"""
        selected = self.folder_var.get().strip()
        if not selected:
            return
        # 如果选择的路径已不在列表中，自动追加并持久化
        folders = list(self._app_cfg.get("folders", []) or [])
        if selected not in folders and os.path.isdir(selected):
            folders.append(selected)
            self._app_cfg["folders"] = folders
            save_config(self._app_cfg)
            self.folder_combo["values"] = folders
        if os.path.isdir(selected):
            self.project_dir = selected
            self._log("📂 切换到文件夹：{}".format(selected), "INFO")
            self._refresh_status()
        else:
            self._log("⚠️ 文件夹不存在：{}".format(selected), "WARN")

    def _cmd_find_folder(self):
        """选择文件夹并加入管理列表，立即持久化到 config.json。"""
        path = filedialog.askdirectory(title="选择文件夹")
        if not path:
            return
        path = os.path.normpath(path)
        if not os.path.isdir(path):
            FloatingToast(self, "⚠️ 路径无效或不是文件夹", "#D32F2F")
            return
        folders = list(self._app_cfg.get("folders", []) or [])
        if path not in folders:
            ...
            # 立即保存到配置文件，防止异常退出丢失
            save_config(self._app_cfg)
            self._log("✅ 已添加文件夹：{}（已保存）".format(path), "SUCCESS")
        else:
            self._log("✅ 使用已有文件夹：{}".format(path), "SUCCESS")
        self.folder_var.set(path)
        self.project_dir = path
        self._refresh_folder_combo()
        self._refresh_status()

    def _cmd_manage_folders(self):
        """弹出文件夹管理对话框：删除、重排。"""
        ...
        tk.Label(frm,
                 text="📦 已保存的 Git 工作文件夹",
                 bg="#E8E8E8", fg="#F47524",
                 font=("Microsoft YaHei", 12, "bold")).pack(anchor="w", pady=(0, 4))

        list_frame = tk.Frame(frm, bg="#E8E8E8")
        list_frame.pack(fill="both", expand=True, pady=(0, 8))

        lst = tk.Listbox(list_frame, bg="#FFFFFF", fg="#4169E1",
                         selectbackground="#ADD8E6", selectforeground="#101010",
        ...
        def _load():
            lst.delete(0, "end")
            for p in self._app_cfg.get("folders", []) or []:
                tag = ""
                if isinstance(p, str) and p.strip():
                    tag = "" if os.path.isdir(p) else "❌"
                lst.insert("end", ("{} {}".format(tag, p)).strip())

        _load()

        def _current_paths():
            return self._app_cfg.get("folders", []) or []

        def _refresh():
            # 从 Listbox 重建列表（保留顺序）
            new_list = []
            for line in lst.get(0, "end"):
                p = line.strip()
                if p.startswith("❌ "):
                    p = p[2:].strip()
                elif p.startswith("❌"):
                ...
                if p:
                    new_list.append(p)
            self._app_cfg["folders"] = new_list
            save_config(self._app_cfg)
            self._refresh_folder_combo()
            _load()

        # 操作按钮行
        op_row = tk.Frame(frm, bg="#E8E8E8")
        op_row.pack(fill="x", pady=4)

        def _delete_selected():
            sel = list(lst.curselection())
            if not sel:
                FloatingToast(top, "⚠️ 请先选择一个或多个文件夹", "#D32F2F")
                return
            if not messagebox.askyesno(
                    "确认删除",
                    "确定从列表中删除选中的 {} 个文件夹吗？\n"
                    "（仅从本程序记录中移除，不会删除磁盘上的文件夹）".format(len(sel)),
                    parent=top):
                return
            for idx in reversed(sel):
                lst.delete(idx)
            _refresh()
            FloatingToast(top, "✅ 已删除")
        ...
        def _add_new():
            path = filedialog.askdirectory(title="选择要添加的文件夹", parent=top)
            if not path: return
            path = os.path.normpath(path)
            if not os.path.isdir(path):
                FloatingToast(top, "⚠️ 路径无效", "#D32F2F"); return
            folders = _current_paths()
            if path not in folders:
                folders.append(path)
                self._app_cfg["folders"] = folders
                save_config(self._app_cfg)
        ...
        def _use_selected():
            ...
            if p and os.path.isdir(p):
                ...
                self._refresh_status()
                top.destroy()
                FloatingToast(self, "✅ 已切换到该文件夹")

        self._mk_btn(op_row, "🗑️ 删除选中", BTN_COLOR1, _delete_selected
                    ).pack(side="left", padx=3)
        self._mk_btn(op_row, "⬆️ 上移", BTN_COLOR1, _move_up
                    ).pack(side="left", padx=3)
        self._mk_btn(op_row, "⬇️ 下移", BTN_COLOR1, _move_down
                    ).pack(side="left", padx=3)
        self._mk_btn(op_row, "➕ 添加文件夹", BTN_COLOR2, _add_new
                    ).pack(side="left", padx=3)
        self._mk_btn(op_row, "🚀 切换使用", BTN_COLOR2, _use_selected
                    ).pack(side="left", padx=3)

        # 底部关闭按钮
        btns = tk.Frame(frm, bg="#E8E8E8")
        btns.pack(fill="x", side="bottom", pady=(8, 0))
        tk.Button(btns, text="❌ 关闭", bg=BTN_COLOR1, fg="white",
                  relief="flat", activebackground=BTN_COLOR1, activeforeground="white",
                  font=("Microsoft YaHei", 10, "bold"), padx=14, pady=5,
    ...
    def _cmd_remote(self):
        ...
        if not url:
            self._run_async(
                lambda: self._exec_git(["remote", "-v"], label="remote -v"))
            return
        self.cfg["remote_url"] = url

        def _do():
            # 优先用 set-url，避免破坏已有 branch 追踪关系
            rc_g, _, _ = run_git(['remote', 'get-url', 'origin'],
                                  cwd=self.project_dir)
            if rc_g == 0:
                run_git(['remote', 'set-url', 'origin', url],
                        cwd=self.project_dir)
                self._log('🔗 origin(更新) -> {}'.format(url), 'INFO')
            else:
                run_git(['remote', 'remove', 'origin'], cwd=self.project_dir)
                self._exec_git(["remote", "add", "origin", url],
                               label="remote add origin")
    ...
    def _github_cli_available(self) -> bool:
        try:
            p_ = subprocess.run([self._find_gh_exe(), '--version'],
    ...
    def _run_gh_auth_status(self) -> tuple:
        """运行 `gh auth status`，返回 (returncode, stdout_combined)。"""
        ...
        try:
            ...
            return p_.returncode, p_.stdout or ""
        except subprocess.TimeoutExpired:
            return -1, "gh auth status timeout"
        except FileNotFoundError:
            return -2, "gh not found: {}".format(gh_exe)
        except Exception as exc:
            return -3, "gh auth status error: {}".format(exc)

    def _github_auth_status(self) -> str:
        """gh auth status wrapper。返回值:
        - 'logged_in'     已登录
        - 'not_logged_in' 未登录
        - 'missing'       gh CLI 未安装
        - 'unknown'       无法判断
        ...
        if not self._github_cli_available():
            return "missing"
        try:
            rc, out_s = self._run_gh_auth_status()
        except Exception:
            return "unknown"
        low = (out_s or "").lower()
        # gh 2.60+ 输出类似 "✓ Logged in as <name>"
        if rc == 0 and ("logged in" in low or "authenticated" in low
                        or "you're logged in" in low):
            return "logged_in"
        # rc != 0 且输出里含 "logged in" 也可能是 gh 的交互提示
        if "logged in" in low:
            return "logged_in"
        # 未登录时 gh 输出:
        ...
        if ("not logged in" in low
                or "you are not logged in" in low
                or rc != 0):
            return "not_logged_in"
        return "unknown"

    def _github_account_info(self) -> str:
        """返回 gh 当前登录账号信息（失败返回空字符串）。"""
        gh_exe = self._find_gh_exe()
        try:
            p_ = subprocess.run(
                [gh_exe, 'api', 'user', '-q', '.login'],
    ...
    def _gh_login_status(self):
        ...
        gh_exe = self._find_gh_exe()
        self._log('🔑 启动 gh 设备码登录（gh = {}）...'.format(gh_exe), 'INFO')
        ...
        # 先等一段时间让 gh 把设备码和 URL 吐出来
        # 同时在每次读取输出后打开浏览器（至少一次）
        _tm.sleep(5)
        _opened_browser = False
        code = ''; url = ''
        for _ in range(6):  # 最多再等 30s，每 5s 读一次
            raw = _raw()
            if not code:
                m_code = _re.search(
                    r'(?:one-time code:|\bcode\b[\s:=]+|device code[\s:=]+)'
                    r'([A-Z0-9-]{4,20})',
                    raw, _re.IGNORECASE)
                if m_code:
            ...
            if not _opened_browser:
                _wb.open(url)
                _opened_browser = True
                self._log('🖥️ 浏览器已打开: {}'.format(url), 'INFO')
                if code:
                    self._log('🔑 请输入一次性验证码: {}'.format(code), 'SUCCESS')
                    FloatingToast(
                        self,
                        '打开 github.com/login/device 输入代码 {} 授权'
                        .format(code),
                        '#2E7D32')
                else:
                    self._log('⚠️ 未能解析验证码，请手动访问 {} 登录'.format(url), 'WARN')
                    FloatingToast(
                        self,
                        '打开 github.com/login/device 授权',
                        '#2E7D32')
            if code:
                break
            if pr.poll() is not None:
                self._log('⚠️ gh login 进程已提前结束（可能已在当前机器完成过）', 'WARN')
                break
            _tm.sleep(5)

        # 轮询：检查设备码登录是否完成（gh 进程退出 或 auth status 变为 logged_in）
        def _poll_loop():
            for i in range(240):  # 约 8 分钟
                _tm.sleep(2)
                # gh 登录进程结束代表设备码授权已完成
                if pr.poll() is not None:
                    _tm.sleep(1)
                    ...
                    if st == 'logged_in':
                        user = self._github_account_info()
                        self._log('✅ gh 登录成功！账号: {}'.format(user or '(未知)'),
                                  'SUCCESS')
                        FloatingToast(self, '✅ gh 登录成功', '#2E7D32')
                        return
                    else:
                        self._log('⚠️ gh 进程结束但未检测到登录状态，请再试一次', 'WARN')
                        return
                # 进程还活着但 auth status 变 logged_in 也可以
                if i % 5 == 0:
                    st = self._github_auth_status()
                    if st == 'logged_in':
                        user = self._github_account_info()
            ...
            if st == 'logged_in':
                ...
                self._log('⚠️ 登录超时，请确认浏览器是否已完成授权', 'WARN')
                try:
    ...
    def _cmd_github_create(self):
        """GitHub 创建远程仓库。未初始化时先自动 init 再打开创建窗口。"""
        if not is_git_repo(self.project_dir):
            self._log("ℹ️ 当前目录不是 Git 仓库，自动先初始化...", "INFO")
            FloatingToast(self, "🔄 正在自动初始化仓库...")
            ok = False
            try:
                rc, _, err = run_git(
                    ["init", "-b", self.cfg.get("default_branch", "main")],
                    cwd=self.project_dir)
                ok = (rc == 0)
            except Exception as ex:
                self._log("❌ init 失败：{}".format(ex), "ERROR")
            if not ok:
                self._log("❌ 自动初始化失败，请先点“初始化仓库”按钮", "ERROR")
                FloatingToast(self, "❌ 自动初始化失败", "#D32F2F")
                return
            self._log("✅ 自动初始化完成", "SUCCESS")
            self._refresh_status()
        ...
        if gh_state == 'logged_in':
            user = self._github_account_info()
            st = '✅ gh 已登录：@{}'.format(user or '(账号查询失败)')
            sf = '#2E7D32'
        elif gh_state == 'not_logged_in':
            st = '⚠️ gh 未登录 — 点击下面 “gh login” 按钮或直接粘贴 PAT Token'; sf = '#D84315'
        elif gh_state == 'missing':
            st = 'ℹ️ gh CLI 未安装 — 可粘贴 PAT Token 代替'; sf = '#555555'
        else:
            st = '❓ gh 状态未知 — 可粘贴 PAT Token'; sf = '#D32F2F'
        tk.Label(
        ...
        e_repo = _mk_entry(4,
            os.path.basename(os.path.normpath(self.project_dir)))
        tk.Label(
        ...
        def _do_create():
            ...
            if method == 'gh':
                ...
                # 只用 gh 建远端仓库，不把当前目录当 source 传进去。
                # gh repo create --source 会尝试自行绑定 origin，与我们自己的
                # _apply_origin_and_push 逻辑冲突且容易失败（已有 origin 时）。
                args = [gh_exe, 'repo', 'create', name_arg]
                ...
                def _run_gh():
                    ...
                    if err:
                        for line in err.splitlines()[:50]:
                            self._log(line, 'WARN')
                    # 从 stdout 里解析仓库地址（gh repo create 会输出 URL）
                    url_clone = ''
                    for line in (out + '\n' + err).splitlines():
                        line = line.strip()
                        if line.startswith('http') and 'github.com' in line:
                            # 去掉反引号/中括号包裹（gh 可能输出 markdown）
                            line = line.strip('`').strip()
                            url_clone = line.rstrip('/')
                            if not url_clone.endswith('.git'):
                                url_clone += '.git'
                            break
                    if not url_clone:
                        # 兜底：自己拼 URL
                        user = self._github_account_info() or owner or ''
                        if user:
                            url_clone = 'https://github.com/{}/{}.git'.format(
                                user.rstrip('/'), repo_name)
                    if rc == 0:
                        self._log(
                            '✅ ok GitHub repo created (gh): {}'.format(url_clone),
                            'SUCCESS')
                        # 本地 init（如尚未初始化）+ 绑定 origin + push
                        if not is_git_repo(self.project_dir):
                            self._log('ℹ️ 自动 init 仓库...', 'INFO')
                            run_git(['init', '-b',
                                    self.cfg.get('default_branch', 'main')],
                                    cwd=self.project_dir)
                        def _do_push():
                            self._apply_origin_and_push(url_clone)
                        self._run_async(_do_push)
                    else:
                        self._log(
                            '❌ gh create failed (exit={})'.format(rc), 'ERROR')
                        if ('already exists' in combined
                                or 'name already exists' in combined
                                or '422' in combined):
                            self._log(
                                'ℹ️ 仓库名已存在，尝试直接绑定 origin 再推送', 'WARN')
                            user = self._github_account_info() or owner or ''
                            if user:
                                url_clone = url_clone or (
                                    'https://github.com/{}/{}.git'.format(
                                        user.rstrip('/'), repo_name))
                            if not url_clone:
                                self._log('❌ 无法推断仓库地址，请手动输入', 'ERROR')
                                self._run_async(self._cmd_remote)
                                return
                            if not is_git_repo(self.project_dir):
                                run_git(['init', '-b',
                                        self.cfg.get('default_branch', 'main')],
                        ...
                        if ('not logged' in combined) or rc == 4:
                            if token and has_curl:
                                self._log('ℹ️ gh 未登录，自动切换 curl...', 'INFO')
                                self._run_curl_create(
                                    repo_name, owner, visibility, desc, token)
                                return
                        self._log(
                            '⚠️ gh 创建失败，请改用 curl + PAT 或手动到网页创建', 'WARN')
                        if not url_clone and token and has_curl:
                            self._run_curl_create(
                                repo_name, owner, visibility, desc, token)
    ...
    def _apply_origin_and_push(self, remote_url):
        ...
        - 如果本地还没有 commit，则自动 add + commit 一次。
        - origin 已存在时改用 set-url，不再 remove+add（避免已有分支追踪关系被破坏）。
        """
        _ensure_github_identity(remote_url, cwd=self.project_dir)
        # 检测 origin 是否已配置
        rc_get, out_get, _ = run_git(['remote', 'get-url', 'origin'], cwd=self.project_dir)
        if rc_get == 0:
            # origin 已存在，只改 URL
            run_git(['remote', 'set-url', 'origin', remote_url], cwd=self.project_dir)
            self._log('🔗 origin -> {}'.format(remote_url), 'INFO')
        else:
            # 先确保不存在（remove 失败也忽略）
            run_git(['remote', 'remove', 'origin'], cwd=self.project_dir)
            ok = self._exec_git(['remote', 'add', 'origin', remote_url],
                                 label="remote add origin")
            if not ok:
                self._log('❌ 无法绑定 origin', 'ERROR')
                return
        rc, out, _ = run_git(["rev-parse", "--verify", "HEAD"], cwd=self.project_dir)
        if rc != 0:
    ...
    def _git_push_status(self):
        ...
        if rc == 0 and out.strip():
            branch = out.strip()
        rc_f, _, _ = run_git(['fetch', 'origin'], cwd=self.project_dir)
        if rc_f != 0:
    ...
    def _cmd_push_async(self):
        ...
        def _doit():
            ...
            _ensure_github_identity(url, cwd=self.project_dir)
            ...
            if rc3 == 0:
                ...
                if not ok:
                    ...
                    if behind > 0:
                        ...
                        rc_pr, out_pr, err_pr = safe_pull_rebase(
                            cwd=self.project_dir, remote='origin',
                            branch=br, log_fn=self._log)
                        if rc_pr != 0:
                            self._log(
                                '❌ pull --rebase 失败，请手动解决冲突后再点推送。', 'ERROR')
                            for line in (out_pr + err_pr).splitlines()[:30]:
                                self._log(line, 'WARN')
                            return
    ...
    def _resolve_remote_then_push(self):
        ...
        def _gh_auto():
            ...
            top.destroy()
            repo_name = os.path.basename(os.path.normpath(self.project_dir))
            def _run():
                gh_exe = self._find_gh_exe()
                # 只用 gh 建远端，不把当前目录当 source（会与已有 origin 冲突）
                args = [gh_exe, 'repo', 'create', repo_name, '--public']
                try:
                    p_ = subprocess.run(
                        args, cwd=PROGRAM_DIR,
                        stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                    ...
                    self._log('gh 调用失败: {}'.format(ex), 'ERROR'); return
                out = p_.stdout or ''; err = p_.stderr or ''
                combined = (out + err).lower()
                if out:
                    for line in out.splitlines()[:50]:
                ...
                for line in (out + '\n' + err).splitlines():
                    ...
                    if line.startswith('http') and 'github.com' in line:
                        ...
                        break
                if not url_clone:
                    user = self._github_account_info() or ''
                    if user:
                        url_clone = 'https://github.com/{}/{}.git'.format(
                            user, repo_name)
                if p_.returncode == 0:
                    ...
                    self._log('gh 创建失败，可能已存在，尝试直接绑定 origin 再推送', 'WARN')
                    if not is_git_repo(self.project_dir):
                        run_git(['init', '-b',
                                self.cfg.get('default_branch', 'main')],
                                cwd=self.project_dir)
    ...
    def _cmd_push(self):
        ...
        rc_head, _, _ = run_git(['rev-parse', '--verify', 'HEAD'],
                                cwd=self.project_dir)
        dirty = _working_tree_is_dirty(self.project_dir)
        if rc_head != 0 or (rc_head == 0 and dirty):
            if rc_head != 0:
                ...
                if not messagebox.askyesno(
                        ...
                        '是否自动 commit + push？'.format(
                            len(_collect_changed_files(self.project_dir))
                            or '若干')):
                    return
            ...
            pruned = _prune_ignored_from_index(self.project_dir)
            if pruned:
                self._log('🗑️ 停止追踪 {} 个过滤文件（.env/chat_history/__pycache__ 等）'.format(len(pruned)),
                          'WARN')
            if rc_head != 0:
                _ensure_gitignore(self.project_dir)
                pruned0 = _prune_ignored_from_index(cwd=self.project_dir)
                if pruned0:
                    self._log('🗑️ 停止追踪 {} 个过滤文件（chat_history/.env 等）'.format(len(pruned0)),
                              'WARN')
                def _first():
                    self._exec_git(['add', '--', '.gitignore'], label='add .gitignore')
                    added, ok, _msg = auto_add_filtered(
                        self.project_dir, log_fn=self._log)
                    if not added:
                        self._log('⚠️ 没有可上传的文件，请先把代码放到项目目录下。', 'WARN')
                        return
                    if len(added) >= 200:
                        self._log('📦 auto add {} 个文件（已过滤安全）'.format(len(added)), 'INFO')
                    else:
                        self._log('📦 auto add {} 个文件'.format(len(added)), 'INFO')
                    if not ok:
                        self._log('❌ 部分文件 add 失败，见上方日志', 'ERROR')
            ...
            def _again():
                _ensure_gitignore(self.project_dir)
                pruned2 = _prune_ignored_from_index(cwd=self.project_dir)
                if pruned2:
                    self._log('🗑️ 停止追踪 {} 个过滤文件（chat_history/.env 等）'
                              .format(len(pruned2)), 'WARN')
                self._exec_git(['add', '--', '.gitignore'], label='add .gitignore')
                added, ok, _m = auto_add_filtered(self.project_dir, log_fn=self._log)
                if not added:
                    self._log('ℹ️ 没有新增或变更文件可上传', 'INFO')
                    return
    ...
    def _push_after_commit(self):
        ...
        git_remote = _read_remote_origin_url(self.project_dir)
        if git_remote:
    ...
    def _cmd_pull(self):
        ...
        def _detect_local_has_head():
            """本地是否已有至少一个 commit。"""
            rc, _, _ = run_git(["rev-parse", "--verify", "HEAD"],
                                cwd=self.project_dir)
            return rc == 0

        def _detect_remote_default_branch(url_hint=None):
            """向 origin 查询远端默认分支名。查不到时回退到 default_branch 配置。"""
            rc, out, _ = run_git(["ls-remote", "--symref", "origin", "HEAD"],
                                 cwd=self.project_dir)
            if rc != 0:
                return self.cfg.get("default_branch", "main")
        ...
        def _prompt_url_and_pull():
            cur = _read_remote_origin_url(self.project_dir) or self.cfg.get("remote_url", "")
            dlg = DialogWithPaste(
                self, "远程仓库",
                "origin 未配置或 pull 失败，请输入远程仓库 URL\n"
                "（直接粘贴 https://github.com/owner/repo.git 或 git@github.com:owner/repo.git）：",
                initial=cur)
            self.wait_window(dlg)
            url = (dlg.result or "").strip()
            if not url:
                self._log('❌ 未输入远程 URL，已取消 pull', 'WARN')
                return
            self.cfg["remote_url"] = url

            def _setup_then_pull():
                rc_g, _, _ = run_git(['remote', 'get-url', 'origin'],
                                      cwd=self.project_dir)
                if rc_g == 0:
                    run_git(['remote', 'set-url', 'origin', url],
                ...
                self._exec_git(["remote", "-v"], label="remote -v")
                _do_pull()
            self._run_async(_setup_then_pull)

        def _do_pull():
            # 远端默认分支
            remote_branch = _detect_remote_default_branch()

            if not _detect_local_has_head():
                # 本地无任何 commit（刚 git init）→ 等同 git clone 行为
                self._log(
                    '🔄 本地仓库无 commit，使用 fetch + checkout 方式拉取 origin/{}'.format(
                        remote_branch), 'INFO')
                self._exec_git(
                    ["fetch", "--depth=200", "origin", remote_branch],
                    label="fetch origin " + remote_branch)
                self._exec_git(
                    ["checkout", "-B", remote_branch, "FETCH_HEAD"],
                    label="checkout origin/" + remote_branch)
                self._exec_git(["status"], label="status")
                FloatingToast(
                    self, '✅ 拉取完成，已检出 origin/{}'.format(remote_branch), '#4CAF50')
                return

            # 正常 pull
            rc, out, _ = run_git(["symbolic-ref", "--short", "HEAD"],
                                  cwd=self.project_dir)
            branch = out.strip() if rc == 0 and out.strip() else remote_branch
            safe_pull_rebase(
                cwd=self.project_dir, remote='origin',
                branch=branch, log_fn=self._log)

        # origin 完全没配置 → 先弹输入框
        if not _read_remote_origin_url(self.project_dir):
            self._log('📡 未配置 origin 远程，请先输入远程仓库 URL...', 'INFO')
            _prompt_url_and_pull()
            return

        # origin 已配置 → 先检查远端连通性，再拉取；失败则弹输入框让用户修正
        def _attempt():
            # 先查远端 HEAD 分支，顺便验证网络可达
            rc0, out0, err0 = run_git(
                ["ls-remote", "--symref", "origin", "HEAD"])
            if rc0 != 0:
                self._log(
                    '❌ 无法访问 origin ({})，请修正远程 URL 或检查网络'.format(rc0),
                    'ERROR')
                # 保留前几行错误信息方便用户看
                for line in (err0 or "").splitlines()[:5]:
                    self._log('    ' + line, 'WARN')
                self._root_after_async(_prompt_url_and_pull)
                return

            # ls-remote 通了，说明 origin 可用；直接拉取
            try:
                _do_pull()
            except Exception as exc:
                self._log('❌ pull 异常: {}'.format(exc), 'ERROR')
                FloatingToast(self, 'pull 异常，请查看日志', '#D84315')

        self._run_async(_attempt)

    def _root_after_async(self, func):
        """跨线程把一个函数投递回主线程执行（用于从异步回调里弹对话框）。"""
        try:
            self.after(0, func)
        except Exception:
            func()
    ...
    def _on_close(self):
        # 保存配置
        self._app_cfg["folders"] = list(self._app_cfg.get("folders", []))
        save_config(self._app_cfg)
        self.destroy()
